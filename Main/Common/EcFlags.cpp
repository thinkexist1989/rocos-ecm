...
DEFINE_int32(perf, 1, "Enable max. and average time measurement in μs for all EtherCAT jobs (e.g. ProcessAllRxFrames). Level: 0 = off, 1 (default) = min/avg/max, 2 = additional histogram");
...
DEFINE_int32(maxbusslaves, 256, "The maximum number of slaves in the EtherCAT network. The default is 256.");

DEFINE_int32(mbxsrv, 0x88A4, "Mailbox server port. The default is 0x88A4.");
...
//
//DEFINE_string(intelgbe, "1 1", "<instance>: Device instance 1=first, 2=second; <mode>: Mode 0 = Interrupt mode, 1= Polling mode");
//static bool ValidateIntelgbe(const char* flagname, const std::string& value) {
//    std::regex ws_re("\\s+"); // whitespace
//    std::vector<std::string> v(std::sregex_token_iterator(value.begin(), value.end(), ws_re, -1), std::sregex_token_iterator());
//    if(v.size() != 2) {
//        printf("Invalid value for --%s: %s\n", flagname, value.c_str());
...
//        return true;
//}
//DEFINE_validator(intelgbe, &ValidateIntelgbe);